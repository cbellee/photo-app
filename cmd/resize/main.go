...
var (
	...
	uploadsQueueBinding     = os.Getenv("UPLOADS_QUEUE_BINDING")
...
func ResizeHandler(ctx context.Context, in *common.BindingEvent) (out []byte, err error) {
	...
	// get env Vars
	mth, err := strconv.Atoi(os.Getenv("MAX_THUMB_HEIGHT"))
	if err != nil {
		Error.Printf("%s: %v", serviceName, err)
	}
	mtw, err := strconv.Atoi(os.Getenv("MAX_THUMB_WIDTH"))
	if err != nil {
		Error.Printf("%s: %v", serviceName, err)
	}
	mih, err := strconv.Atoi(os.Getenv("MAX_IMAGE_HEIGHT"))
	if err != nil {
		Error.Printf("%s: %v", serviceName, err)
	}
	miw, err := strconv.Atoi(os.Getenv("MAX_IMAGE_WIDTH"))
	if err != nil {
		Error.Printf("%s: %v", serviceName, err)
	}
	...
	_, err = setBlob(ctx, thumbsContainerBinding, blobName, thumbBytes, blob.Metadata["collection"], blob.Metadata["collection"])
	if err != nil {
